...
def overall_ner_performance(prediction_dir: str) -> pd.DataFrame:
    ...
    for model in performance_report:
        for condition in performance_report[model]:
            metrics = performance_report[model][condition]
            row = {
                'model': model,
                'condition': condition,
            }
...
def main():
    ...
    make_few_shot_box_plot(all_class, 'few_shot/overall_class_performance_box_plot.png',
                         metric='f1-weighted')
    make_few_shot_trend_plot(all_class, 'F1 Score for 0-, 1-, 3-, 5-shot over All Tasks',
                             'few_shot/overall_class_trend_plot_sharedy.png')
    make_few_shot_trend_plot(all_class, 'F1 Score for 0-, 1-, 3-, 5-shot over All Tasks',
                             'few_shot/overall_class_trend_plot.png', sharey=False)
    make_few_shot_delta_plot(all_class, 'Relative Improvement Δ F1 for 1-, 3-, 5-shot over Zero-Shot for All Tasks',
                             'few_shot/overall_class_relative_delta_plot.png')
    make_few_shot_avg_plot(all_class, 'Average Performance over All Tasks',
                            'few_shot/overall_class_average_performance_plot.png')
    make_few_shot_parallel_plot(
         all_class, 'In-context Learning Performance across Models and Conditions', 'few_shot/overall_class_parallel_plot.png')
