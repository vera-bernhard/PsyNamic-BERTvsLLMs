...
def overall_ift_performance(prediction_dirs: str, metric: str) -> pd.DataFrame:
    ...
    best_models = []
    df_best = df_all_tasks.loc[df_all_tasks.groupby('task')[metric].idxmax()]
...
def overall_ift_ner_performance(prediction_dirs: str, lst_dir: str, metric: str='f1 overall - strict') -> pd.DataFrame:
    ...
    overall_ner_data, error_df = overall_ner_performance(prediction_dir='few_shot')
    # remove bert and tuned models from overall_ner_data
    overall_ner_data = overall_ner_data[~overall_ner_data['model'].str.contains('bert', case=False)]
    overall_ner_data = overall_ner_data[~overall_ner_data['model'].str.contains('tuned', case=False)]
    # get best model-condition combination, based on the specified metric
    best_row = overall_ner_data.loc[
        overall_ner_data[overall_ner_data["metric"].str.lower() == metric.lower()]["mean"].idxmax()
    ]
    best_model = {
    ...
    return pd.DataFrame(rows), pd.DataFrame([best_model])
...
def evaluate_lst(lst_dir: str):
    ...
    pred_bio, pred_entities, true_bio, true_entities = get_ner_predictions_and_labels(file)
...
def main():
    ...
    all_ift, best_models = overall_ift_performance(PRED_DIRS, metric='f1-weighted')
    make_ift_class_performance_plot(all_ift, best_models, title='Best ICL Model vs. IFT Model vs. BERT Baseline', save_path=os.path.join(OUTPUT_DIR, 'ift_class_performance.png'))
    all_ift_ner, best_model_ner = overall_ift_ner_performance(PRED_DIRS, LST_DIR,  metric='f1 overall - strict')
    make_ift_class_performance_plot(all_ift_ner, best_model_ner, title='Best ICL Model vs. IFT Model vs. LST  Model vs. BERT Baseline', save_path=os.path.join(OUTPUT_DIR, 'ift_ner_performance.png'), metric='f1 overall - strict')
