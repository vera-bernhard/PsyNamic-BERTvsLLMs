"""
Filename: lst_predict.py
Description: Script to load a label-supervised fine-tuned language model for named entity recognition and make predictions on the test set.
...
parser.add_argument("--model_dir", type=str,
                    default="./finetuning/lst_llama3_8b")
...
# Make predictions on test set, remapping subword predictions to token-level predictions
for i, row in df_test.iterrows():
    ...
    if gold_len != len(preds):
        print(
            f"Row {i} has mismatched lengths: gold({gold_len}) vs pred({len(preds)})")

    df_test.at[i, "pred_labels"] = str(preds)
    df_test.at[i, "bert_tokens"] = str(bert_tokens)
    df_test.at[i, "bert_ner_tags"] = str(
        [id2label[int(pid)] for pid in pred_ids])
